...
export class DashboardComponent implements OnInit {
  ...
  runReport(report) {
   ...
   Error => {
    ...
    }else if(this.reportView.reportName ==="Employee expenses Report"){
        this.viewReportService.getCustomReport('generateExpensesReport').subscribe(response =>{
        // console.log(response);
         if (response.status = 200) {
            localStorage.setItem("report", JSON.stringify(this.reportView));
            localStorage.setItem("viewResponse", response._body);
   ...
   Error => {
    ...
    } else  if(this.reportView.reportName ==="Employee leaves Report"){
        this.viewReportService.getCustomReport('generateLeavesReport').subscribe(response =>{
        // console.log(response);
         if (response.status = 200) {
            localStorage.setItem("report", JSON.stringify(this.reportView));
            localStorage.setItem("viewResponse", response._body);
